...
class FashionIQTrainValDataset(FashionIQDataset):
    ...
    def __load_data__(self):
       ...
       # with open('/Users/yuanyifei/CVPR/assets/sentence_embedding/embeddings_2.pkl','rb') as f:
            self.we = pickle.load(f)
        with open('/projdata1/info_fil/yfyuan/task202008/CVPR/assets/image_embedding/embeddings_2.pkl', 'rb') as f:
        #with open('/Users/yuanyifei/CVPR/assets/image_embedding/embeddings_2.pkl', 'rb') as f:
        ...
        for t in targets:
            ...
            tag_file = 'asin2attr.{}.{}.new.json'.format(t,self.split)
            print('[Dataset] load multiturn annotation file: {}'.format(cap_file))
            full_cap_path = os.path.join(self.data_root+'Multiturn/',cap_file)
            full_tag_path = os.path.join(self.data_root+'image_tag_dataset-master/tags',tag_file)
            print(full_cap_path)
            assert os.path.exists(full_cap_path)
            with open(full_tag_path,'r') as f:
            ...
            with open(full_cap_path,'r') as f:
                ...
                for i,d in enumerate(tqdm(data)):
                    ...
                    for references in ref_list:
                        ...
                        if c_id in tags_dict:
                            tags = tags_dict[c_id][1:]
                        else:
                            tags = [[],[],[],[],[],[]]
    ...
    def __sample__(self,index):
        ...
        for ref in reference:
            ...
            reference_list.append([x_c,c_c,text,target_list])
        # multiturn padding
        ...
        reference_list.append((x_t,c_t,data['t_id'],target_tag))
...
class FashionIQTestDataset(FashionIQDataset):
    ...
    def __load_data__(self):
        ...
        tag_file = 'asin2attr.{}.{}.new.json'.format(self.target,self.split)
        full_tag_path = os.path.join(self.data_root + 'image_tag_dataset-master/tags', tag_file)
    ...
    def __sample_query__(self, index):
        ...
        for ref in reference:
            ...
            x_c = self.__load_pil_image__(img_path)
            c_c = self.cls2idx[c_id]
            reference_list.append([x_c, c_c, text, tag])
        if not self.transform is None:
            for i in range(len(reference_list)):
                reference_list[i][0] = self.transform(reference_list[i][0])
            x_t = self.transform(x_t)
        ...
        for i in range(len(reference_list)):
            reference_list[i] = tuple(reference_list[i])
        reference_list.append((x_t,c_t,data['t_id'],data['target_tag']))
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--image_root', default='/Users/yuanyifei/CVPR/data/', type=str)
