...
class NormalizationLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        return features
class MultiturnNormalizationLayer(torch.nn.Module):
    def __init__(self,normalize_scale=1.0,learn_scale=True):
        super(MultiturnNormalizationLayer,self).__init__()
        self.norm_s = float(normalize_scale)
        if learn_scale:
            self.norm_s = torch.nn.Parameter(torch.FloatTensor((self.norm_s,)))
    def forward(self, x):
        features = self.norm_s * x/torch.norm(x,dim=2,keepdim=True).expand_as(x)
        return features
class BatchHardTripleLoss(nn.Module):
    def __init__(self,margin=1,p=2,squared=True,soft=False):
        super(BatchHardTripleLoss,self).__init__()
        self.margin = margin
        self.p = p
    ...
    def forward(self, x1, x2):
        ...
        max_anchor_positive_dist = torch.max(pairwise_dist,dim=1,keepdim=True)[0]
        anchor_negative_dist = pairwise_dist+max_anchor_positive_dist*(1.0-mask_anchor_negative)
...
class SequencialMatching(nn.Module):
    def __init__(self, args):
        super(SequencialMatching,self).__init__()
        self.args = args
        self.linear_layer = nn.Linear(args.max_turn_len,2)
    ...
    def forward(self, logits):
        loss = nn.BCELoss()
        # logits = self.last_layer(matching_scores)
        # total_loss = (1.0-logits.mean())/2
        #logits = self.softmax_layer(self.linear_layer(matching_scores))
        y_true = torch.ones(logits.size()[0]/2,dtype=torch.float32).cuda()
        y_false = torch.zeros(logits.size()[0]/2,dtype=torch.float32).cuda()
        y_final = torch.cat([y_true,y_false],dim=0)
        total_loss = loss(logits,y_final)
        return total_loss
